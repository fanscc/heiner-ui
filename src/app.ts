...
import * as kebabCaseTAGS from 'heiner-new/heiner-tags.json';
import * as kebabCaseATTRS from 'heiner-new/heiner-attributes.json';
...
for (const key in kebabCaseATTRS) {
  if (kebabCaseATTRS.hasOwnProperty(key)) {
    const heiner = kebabCaseATTRS[key];
    ATTRS[key] = heiner;
    ...
    if (hasTag) {
      ...
      attr = tagAttrs[1];
      ATTRS[tag + attr] = JSON.parse(JSON.stringify(heiner));
    }
  }
}
...
export const SCHEME = 'heiner-helper';
...
export class App {
  ...
  openDocs(query?: Query, title = 'Heiner-helper', editor = window.activeTextEditor) {
  ...
  return `
    ...
    <script>
      window.addEventListener('message', (e) => {
        e.data.loaded && (document.querySelector('.heiner-helper-loading-mask').style.display = 'none');
      }, false);
    </script>
    </body>`;
};
...
export class ElementCompletionItemProvider implements CompletionItemProvider {
  ...
  buildTagSuggestion(tag, tagVal, id) {
    ...
    return {
      ...
      detail: `heiner-ui ${tagVal.version ? `(version: ${tagVal.version})` : ''}`,
  ...
  buildAttrSuggestion({ attr, tag, bind, method }, { description, type, version }) {
    if ((method && type === "method") || (bind && type !== "method") || (!method && !bind)) {
      return {
        ...
        detail: tag ? `<${tag}> ${version ? `(version: ${version})` : ''}` : `heiner-ui ${version ? `(version: ${version})` : ''}`,
